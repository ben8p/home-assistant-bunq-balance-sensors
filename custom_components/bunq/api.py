...
def _setup_context():
    ...
    try:
        # setup api context
        installation_response = requests.post(_host + '/v1/installation', data = json.dumps({'client_public_key': public_key_client}), headers = {'Content-Type': 'application/json', 'User-Agent': 'HomeAssistant', 'X-Bunq-Language': 'en_US', 'X-Bunq-Region': 'nl_NL', 'X-Bunq-Client-Request-Id': _request_id, 'X-Bunq-Geolocation': _GEOLOCATION, 'X-Bunq-Client-Signature': ''})
    ...
    try:
        device_server_response = requests.post(_host + '/v1/device-server', data = json.dumps({'description': 'Home Assistant', 'secret': _api_key, 'permitted_ips': _permitted_ips}), headers = {'Content-Type': 'application/json', 'User-Agent': 'HomeAssistant', 'X-Bunq-Language': 'en_US', 'X-Bunq-Region': 'nl_NL', 'X-Bunq-Client-Request-Id': _request_id, 'X-Bunq-Geolocation': _GEOLOCATION, 'X-Bunq-Client-Signature': '', 'X-Bunq-Client-Authentication': installation_token})
    ...
    try:
        ...
        session_server_response = requests.post(_host + '/v1/session-server', data = body, headers = {'Content-Type': 'application/json', 'User-Agent': 'HomeAssistant', 'X-Bunq-Language': 'en_US', 'X-Bunq-Region': 'nl_NL', 'X-Bunq-Client-Request-Id': _request_id, 'X-Bunq-Geolocation': _GEOLOCATION, 'X-Bunq-Client-Signature': signature, 'X-Bunq-Client-Authentication': installation_token})
...
def _fetch_monetary_accounts():
    try:
        response = requests.get(_host + '/v1/user/' + str(_user_id) + '/monetary-account', headers = {'Content-Type': 'application/json', 'User-Agent': 'HomeAssistant', 'X-Bunq-Language': 'en_US', 'X-Bunq-Region': 'nl_NL', 'X-Bunq-Client-Request-Id': _request_id, 'X-Bunq-Geolocation': _GEOLOCATION, 'X-Bunq-Client-Signature': '', 'X-Bunq-Client-Authentication': _session_token})
    except requests.exceptions.HTTPError as http_error:
...
def _fetch_monetary_account_transactions(account_id):
    try:
        response = requests.get(_host + '/v1/user/' + str(_user_id) + '/monetary-account/' + str(account_id) + '/payment', headers = {'Content-Type': 'application/json', 'User-Agent': 'HomeAssistant', 'X-Bunq-Language': 'en_US', 'X-Bunq-Region': 'nl_NL', 'X-Bunq-Client-Request-Id': _request_id, 'X-Bunq-Geolocation': _GEOLOCATION, 'X-Bunq-Client-Signature': '', 'X-Bunq-Client-Authentication': _session_token})
    except requests.exceptions.HTTPError as http_error:
        ...
        _LOGGER.error('Error with payment api (too_many_redirects_error): %s', too_many_redirects_error)
        raise PlatformNotReady
    except requests.exceptions.RequestException as request_exception:
        _LOGGER.error('Error with payment api (request_exception): %s', request_exception)
        raise PlatformNotReady
    except:
